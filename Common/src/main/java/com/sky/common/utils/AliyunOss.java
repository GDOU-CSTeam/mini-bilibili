...
public class AliyunOss {
    ...
    OSS ossClient = new OSSClientBuilder().build(endpointForOss,accessKeyId,accessKeySecret);
    String bucketName = "videotest5";

    public HashMap<String, Object> getVideoKey(String id,String fileSuffix) throws ClientException {
        ...
        String fileName = "src/video/"+ id + "." + fileSuffix;
        String policy = "{\n" +
                "    \"Version\": \"1\", \n" +
                "    \"Statement\": [\n" +
                "        {\n" +
    ...
    public HashMap<String, Object> getImageKey(String id,String fileSuffix) throws ClientException {
        // 以下Policy用于限制仅允许使用临时访问凭证向目标存储空间examplebucket下的src目录上传文件。
        // 临时访问凭证最后获得的权限是步骤4设置的角色权限和该Policy设置权限的交集，即仅允许将文件上传至目标存储空间examplebucket下的src目录。
        // 如果policy为空，则用户将获得该角色下所有权限。
        String fileName = "src/img/" + id +"." + fileSuffix;
        String policy = "{\n" +
                "    \"Version\": \"1\", \n" +
                "    \"Statement\": [\n" +
                "        {\n" +
